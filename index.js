...
const { help } = require('./NANAMI/help')
const { rules } = require('./NANAMI/rules')
const { bahasa } = require('./NANAMI/bahasa')
const { donasi } = require('./NANAMI/donasi')
const { limits } = require('./NANAMI/limit')
const { bokep } = require('./NANAMI/bokep.js')
const { ownermenu } = require('./NANAMI/ownermenu.js')
const { groupmenu } = require('./NANAMI/groupmenu.js')
const { mediamenu } = require('./NANAMI/mediamenu.js')
const { nsfwmenu } = require('./NANAMI/nsfwmenu.js')
const { listmenu } = require('./NANAMI/listmenu.js')
const { funmenu } = require('./NANAMI/funmenu.js')
const { makermenu } = require('./NANAMI/makermenu.js')
//jangan hiraukan ini!!,,,ini uji coba yang gagal:) kontol//
//const { gabutmenu } = require('./NANAMI/gabutmenu')
//const { ownermenu } = require('./NANAMI/ownermenu')
//const { groupmenu } = require('./NANAMI/groupmenu')
//const { vipmenu } = require('./NANAMI/vipmenu')
//const { nsfwmenu } = require('./NANAMI/nsfwmenu')
//const { creatormenu } = require('./NANAMI/creatormenu')
//const { downloadmenu } = require('./NANAMI/downloadmenu')
//const { allmenu } = require('./NANAMI/allmenu')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Aidil Tipi\n'//GANTI NAMA LU COK
            + 'ORG:《Bot Nanami-Chan》;\n'//GANTI NAMA LU!!
            + 'TEL;type=CELL;type=VOICE;waid=62895704959080:+62 895-704959080\n'//GANTI NOMOR LU
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('BURUAN SCAN!! Subscribe Aidil Tipi'))
	})

	fs.existsSync('./Aidil Tipi.json') && client.loadAuthInfo('./Aidil Tipi.json')
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Aidil Tipi.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const AidilTipi = ["62895704959080@s.whatsapp.net"]
			...
			const ownerNumber = [
			"62895704959080@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"62895704959080@s.whatsapp.net","6287749397861@s.whatsapp.net"//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
     ...
     const bayarLimit = (sender, amount) => {
		...
		case 'daftar':
			...
			case 'nanami':
                ...
                setTimeout( () => {
					costum(help(prefix, instagram, yt, name, pushname2, user, limitt), text, AidilTipi, replySet)
					}, 1500)
					setTimeout( () => {
					/*client.sendMessage(from, rules(name, uptime, tanggal, jam, prefix), text, {quoted: mek })
					}, 1200)
					setTimeout( () => {*/
					client.sendMessage(from, 'Terimakasih sudah menggunakan *Nanami Bot* \n\n*SEBAGAI PENGGANTI DONASI YUK SUPORT*\nSubscribe https://youtube.com/c/AidilTipi \nFollow instagram https://instagram.com/aidil.tipi', text, {quoted: mek})
					}, 0)
    				break
/*********PISAH MENU**********/
              case 'ownermenu':
		    ...
		    case 'map':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *NANANMIBOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu!*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[NANAMIBOT]','Yellow'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
